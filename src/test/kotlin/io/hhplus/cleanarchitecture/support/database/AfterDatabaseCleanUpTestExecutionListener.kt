...
class AfterDatabaseCleanUpTestExecutionListener : AbstractTestExecutionListener() {
    ...
    private fun getTruncateQueries(jdbcTemplate: JdbcTemplate): List<String> {
        return jdbcTemplate.queryForList(
            "SELECT Concat('TRUNCATE TABLE ', TABLE_NAME, ';') AS q FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '${TARGET_TABLE_SCHEMA}'",
            String::class.java
        )
    }
    ...
    companion object {
        private const val TARGET_TABLE_SCHEMA = "hhplus-clean-architecture-lecture"
    }
}